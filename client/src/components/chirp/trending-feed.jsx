import { useState, useEffect } from "react";
import PostCard from "../../components/chirp/post-card.jsx";
import useTrendingPosts from "../../hooks/useTrendingFeed.js";
import { useAuth } from "../../contexts/AuthContext";
import { Spinner } from "../../components/ui/spinner";
import axios from "axios";

export default function TrendingFeed() {
  const { posts: initialPosts, loading, error } = useTrendingPosts();
  const { user, token } = useAuth();
  const [posts, setPosts] = useState([]);

  useEffect(() => {
      // eslint-disable-next-line react-hooks/set-state-in-effect
    if (initialPosts) setPosts(initialPosts);
  }, [initialPosts]);

  // Optimistic post removal
  const handleRemovePost = async (postId) => {
    const removedPost = posts.find((p) => p.id === postId);
    setPosts((prev) => prev.filter((p) => p.id !== postId));

    try {
      await axios.delete(`${import.meta.env.VITE_API_URL}/api/posts/${postId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
    } catch (err) {
      console.error("Failed to delete post", err);
      setPosts((prev) => [...prev, removedPost]); // rollback
    }
  };

  if (loading && user)
    return (
      <div className="flex flex-col justify-center items-center m-auto h-screen">
        <Spinner className={"size-20"} />
      </div>
    );

  if (!token)
    return (
      <div className="flex flex-col justify-center items-center m-auto h-screen">
        <p className="text-center">Please log in to see posts</p>
      </div>
    );

  if (error)
    return (
      <div className="flex flex-col justify-center items-center m-auto h-screen">
        <p className="text-center">Error loading posts</p>
      </div>
    );

  if (posts.length === 0)
    return (
      <div className="flex flex-col justify-center items-center m-auto h-screen">
        <p className="text-center text-gray-500">No trending posts yet.</p>
      </div>
    );

  return (
    <main className="max-w-2xl mx-auto px-4 py-6 space-y-4">
      {posts.map((post) => (
        <PostCard key={post.id} post={post} onDelete={handleRemovePost} />
      ))}
    </main>
  );
}
